...
Page {
    ...
    function _normalizeDiaryArray(obj) {
        ...
        if (Array.isArray(obj))
            return obj;
        if (obj.diaries && Array.isArray(obj.diaries))
            return obj.diaries;
        return [];
    }
    ...
    function _mergeDiaryAndStatus(diaryJson, statusJson) {
        ...
        if (Array.isArray(statusObj))
            statuses = statusObj;
        else if (Array.isArray(statusObj.statuses))
            statuses = statusObj.statuses;
        else if (Array.isArray(statusObj.status))
            statuses = statusObj.status;
        ...
        return {
            diaries: diaries,
            statusDates: Object.keys(statusByDate),
    ...
    function applySearchResults(jsonString) {
        ...
        if (Array.isArray(obj)) {
            ...
            for (var gi = 0; gi < obj.groups.length; gi++) {
                ...
                for (var ii = 0; ii < items.length; ii++) {
                    var it = items[ii];
                    if (!it.createdAt && g.date)
                        it.createdAt = g.date + " 00:00:00";
                    rows.push(it);
                }
            }
        } else {
        ...
        for (var i = 0; i < rows.length; i++) {
            ...
            if (r.status) {
                if (r.status.mood !== null && r.status.mood !== undefined)
                    item.mood = r.status.mood;
                if (r.status.freeMoodText !== null && r.status.freeMoodText !== undefined)
                    item.freeMoodText = r.status.freeMoodText;
                if (r.status.sleepTime !== null && r.status.sleepTime !== undefined)
                    item.sleepTime = r.status.sleepTime;
                if (r.status.wakeUpTime !== null && r.status.wakeUpTime !== undefined)
                    item.wakeUpTime = r.status.wakeUpTime;
                if (r.status.temperature !== null && r.status.temperature !== undefined)
                    item.temperature = r.status.temperature;
            }
            item.raw = r;
    ...
    RowLayout {
        ...
        ActivityBar {
            ...
            onActivated: function (key) {
                mainUserPage.handleActivityChange(key);
            }
        }
        ...
        Frame {
            ...
            StackLayout {
                id: sideStack
                anchors.fill: parent
                currentIndex: activityLoader.currentKey === "settings" ? 1 : activityLoader.currentKey === "search" ? 2 : activityLoader.currentKey === "graph" ? 3 : 0
            ...
            GridView {
                ...
                delegate: Rectangle {
                    ...
                    readonly property bool today: {
                        var now = new Date();
                        return day > 0 && now.getDate() === day && now.getMonth() === monthGrid.month && now.getFullYear() === monthGrid.year;
                    }
                ...
                function hasStatusForDay(day) {
                    ...
                    for (var i = 0; i < items.length; i++) {
                        if (items[i] && (items[i].hasStatus === true || items[i].status))
                            return true;
                    }
                    return false;
                }
                ...
                function _extractStatusFields(statusObj) {
                    ...
                    var freeText = statusObj.free_mood_text !== undefined ? statusObj.free_mood_text : (statusObj.freeMoodText !== undefined ? statusObj.freeMoodText : null);
                    var sleep_time = statusObj.sleep_time !== undefined ? statusObj.sleep_time : (statusObj.sleepTime !== undefined ? statusObj.sleepTime : null);
                    var wake_up_time = statusObj.wake_up_time !== undefined ? statusObj.wake_up_time : (statusObj.wakeUpTime !== undefined ? statusObj.wakeUpTime : null);
                    var temperature = statusObj.temperature !== undefined ? statusObj.temperature : null;
                    return {
                        mood: mood,
                        freeTextMood: freeText,
                        ...
                        for (var k = 0; k < statusDates.length; k++) {
                            ...
                            if (ymd.y === mainUserPage.currentYear && ymd.m === monthGrid.month) {
                                ...
                                for (var t = 0; t < items.length; t++) {
                                    if (items[t] && (items[t].hasStatus === true || items[t].status)) {
                                        hasAnyStatus = true;
                                        break;
                                    }
                                }
    ...
    Component {
        id: cmpNoCreateDiary
        MenuItem {
            text: qsTr("Cannot create diary for past days")
            enabled: false
        }
    }
    ...
    Component {
        ...
        MenuItem {
            ...
            onTriggered: {
                dayContextMenu.close();
                mainUserPage.requestStatusEditor(mainUserPage.currentYear, mainUserPage.currentMonth, dayContextMenu.selectedDay, dayContextMenu.selectedStatusJson, mainUserPage.workspacePath, 1);
            }
        }
    }
    Component {
    ...
    Component {
        ...
        MenuItem {
            ...
            onTriggered: {
                dayContextMenu.close();
                mainUserPage.requestStatusEditor(mainUserPage.currentYear, mainUserPage.currentMonth, dayContextMenu.selectedDay, dayContextMenu.selectedStatusJson, mainUserPage.workspacePath, 0);
            }
        }
    }
        ...
        Column {
            ...
            Row {
                ...
                Button {
                    ...
                    text: qsTr("Search")
                    onClicked: {
                        mainUserPage.requestSearch(queryField.text, scopeBox.currentText, caseSensitiveBox.checked, regexBox.checked, mainUserPage.workspacePath);
                    }
                }
                Button {
                    text: qsTr("Clear")
            ...
            Popup {
                ...
                contentItem: Column {
                    ...
                    padding: 8
                    Label {
                        text: qsTr("Operators: & = AND, | = OR, <> = not equal, () grouping")
                    }
                    Label {
                        text: qsTr("Comparisons: =, !=(<>), <, <=, >, >=")
                    }
                    Label {
                        text: qsTr('Example: title:"work" & (mood>3 | temperature>=37)')
                    }
                }
            }
        }
                ...
                Button {
                    ...
                    onClicked: {
                        ...
                        for (let i = 0; i < condModel.count; i++) {
                            ...
                            const v = (c.op === "contains" || c.op === "not contains") ? `"${c.value}"` : c.value;
                            parts.push(`${c.field} ${c.op} ${v}`);
                            if (i < condModel.count - 1)
                                parts.push(c.logic === "OR" ? "|" : "&");
                        }
