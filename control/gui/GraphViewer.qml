...
Page {
    ...
    function updateGraph() {
        ...
        for (var i = 0; i < keys.length; ++i) {
            ...
            if (cfg && cfg.yRange && cfg.yRange.length === 2) {
                ...
                var swRange = clampRange1pt(Math.max(0, Math.floor(minY) - 1), Math.min(24, Math.ceil(maxY) + 1), 1, 2);
        ...
        Repeater {
            ...
            delegate: ChartView {
                ...
                ValueAxis {
                    ...
                    labelFormat: modelData.accessor === "temperature" ? "%.1f" : "%d"

                    tickType: ValueAxis.TicksFixed
                    tickInterval: 1.0

                    minorTickCount: modelData.accessor === "temperature" ? 9 : 0

                    tickAnchor: modelData.accessor === "temperature" ? Math.floor(min) : 0
                ...
                Component.onCompleted: {
                    graphViewerPage.registerSeries(modelData.accessor, lineSeries, axisX, axisY, modelData);
                }
    ...
    function populateSeries(key) {
        ...
        for (var i = 0; i < graphData.days.length; ++i) {
            ...
            if (!day || !day.status || day.status[key] === undefined)
                continue;
